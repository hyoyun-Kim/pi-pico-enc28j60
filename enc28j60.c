...
uint8_t enc28j60ReadOp(uint8_t op, uint8_t address)
{
	...
	//printf("enter the enc28j60readop\n");

	// do dummy read if needed (for mac and mii, see datasheet page 29)
	if (address & 0x80)
	{
		printf("enter the dummy read\n");
		spi_read_blocking(spi_default, 0, dst, 1);
	}
...
void enc28j60ReadBuffer(uint16_t len, uint8_t *data)
{
	...
	printf("enter the enc28j60read *Buffer*\n");
...
void enc28j60WriteBuffer(uint16_t len, uint8_t *data)
{
	...
	// //add(3)
	// channel_config_set_read_increment(&rx_dma_channel_config, false);
	// channel_config_set_write_increment(&rx_dma_channel_config, false);
	// dma_channel_configure(
	// 	rx_dma_chan, &rx_dma_channel_config,
...
void enc28j60Init(uint8_t *macaddr)
{
	...
	// set Full-Duplex Mode (add 21_10_25)
	...
	channel_config_set_dreq(&rx_dma_channel_config, DREQ_SPI0_RX);
	...
	channel_config_set_dreq(&tx_dma_channel_config, DREQ_SPI0_TX);
